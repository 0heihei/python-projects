...
class Log:
    ...
    def reg_or_log(self):
        ...
        if self.getName == '' or self.getPswd == '':  # 用户未输入信息
            tkinter.messagebox.showwarning(self.r, message='请输入账号或密码！')
            self.re_enter()
            return
        elif not re.match('[a-zA-Z0-9]', self.getPswd):  # 用正则判断格式
            tkinter.messagebox.showwarning(self.r, message='密码必须是字母和数字的组合！')
            self.re_enter()
            return
                ...
                try:  # 下面操作字符串有时候会报错(转码错误)，故try
                    if self.v1.get() == 0:
                        if self.userInfo in self.userList:
                            tkinter.messagebox.showerror(message='用户名已存在!')
                            return  # 可以防止窗口关闭，使其停留在登录界面
                        else:
                            tkinter.messagebox.showinfo(self.r, message=' 成功注册！')
                            self.f.write(self.userInfo)
                            self.state = True
                    elif self.v1.get() == 1:
                        if self.userInfo in self.userList:
                            tkinter.messagebox.showinfo(self.r, message='成功登录！')
                            self.state = True
                        else:
                            tkinter.messagebox.showerror(self.r, message='用户名或密码错误！')
                            self.re_enter()
                            return
                finally:
                    self.f.close()
...
class MyApp:
    def __init__(self):
        ...
        self.ft = tkinter.font.Font(font=('Microsoft YaHei', 12, 'normal'))  # 字体默认微软雅黑
    ...
    def getFiles(self, name):  # 打开相应词库文件，读取所有行(每行只有一个单词及其翻译)
        if name == '':  # 不指定词库文件，给出文件对话框供用户选择
            self.filename = tkinter.filedialog.askopenfilename(filetypes=[("all files", "*")], initialdir='.')
            try:
            ...
            if self.filename == ():
                tkinter.messagebox.showwarning('Warning', "please select the file!")
                return
            else:
                tkinter.messagebox.showinfo('Info', 'Open' + str(self.filename.encode('utf8')) + 'successfully')
        else:
            self.filename = './VocBank/' + name  # linux不认双反斜杠
            self.f1 = open(self.filename, 'r')
            self.lines = self.f1.readlines()
    ...
    def wordCompletion(self):
        if self.getWords() == None:  # 防止用户未选词库就点击题型
            tkinter.messagebox.showerror(message='请先选择词库！')
            return
        tkinter.messagebox.showwarning(title='Tips', message='输完按 回车键 即可判断并切换')
        ...
        def judge():  # 判断
            ...
            self.count_of_qs += 1
            ques()
            self.l3 = Label(self.f3, text='   <错词展示栏>    ' + str(self.count_of_mis) + '/' + str(self.count_of_qs),
                            font=self.ft, fg='yellow', bg='DimGray')  # 更新  错误数/总题数
    ...
    def wordChoice(self):
        if self.getWords() == None:  # 防止用户未选词库就点击题型
            tkinter.messagebox.showerror(message='请先选择词库！')
            return
        tkinter.messagebox.showwarning(title='Tips', message='对着选项双击左键即可判断并切换')
        ...
        def judge():  # 判断
            ...
            ques()
            self.count_of_qs += 1
            self.l4 = Label(self.f3, text='   <错词展示栏>    ' + str(self.count_of_mis) + '/' + str(self.count_of_qs),
                            font=self.ft, fg='yellow', bg='DimGray')  # 更新  错误数/总题数
    ...
    def searchWord(self):
        def search_it():
            ...
            trans = self.haici(word)
            if trans != None:
                tkinter.messagebox.showinfo(title='查询结果', message=trans)
            else:
                tkinter.messagebox.showinfo(title='查询结果', message='查无此词！')
            self.v5.set('')
    ...
    def analyseText(self, referenceText):
        self.filename = tkinter.filedialog.askopenfilename(
            filetypes=[("all files", "*")], initialdir='.')
        try:
        ...
        tkinter.messagebox.showinfo(message='析出单词共' + str(self.count_of_words) + '条\n请在 word_of_text.txt 中查看')
